...
export const getJob = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const job = await getJobById(req.params['id'] as string, req.user.campus);
    if (!job) {
...
export const addJob = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { title, company, location, stipend, openings, applyBy, type, status, logo } = req.body as {
      title: string;
      company: string;
      location: string;
      stipend: string;
    ...
    if (!location) errors.push('Job location is required.');
    if (!stipend) errors.push('Stipend information is required.');
    if (openings === undefined || openings === null) errors.push('Number of openings is required.');
...
export const editJob = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const job = await updateJob(req.params['id'] as string, req.user.campus, {
      title,
      company,
      location,
      stipend,
...
export const removeJob = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const job = await deleteJob(req.params['id'] as string, req.user.campus);
    if (!job) {
      res.status(HttpStatus.NOT_FOUND).json({ message: 'Job not found or not in your campus.' });
      return;
    }
