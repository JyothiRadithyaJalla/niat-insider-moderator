import { getAnnouncementsByCampus, getAnnouncementById, createAnnouncement, updateAnnouncement, deleteAnnouncement, } from '../services/announcement.service.js';
...
export const getAnnouncement = async (req, res) => {
    try {
        ...
        const announcement = await getAnnouncementById(req.params['id'], req.user.campus);
        if (!announcement) {
...
export const editAnnouncement = async (req, res) => {
    try {
        ...
        const { title, content, type, pinned } = req.body;
        const announcement = await updateAnnouncement(req.params['id'], req.user.campus, {
            title,
            content,
            type,
            pinned,
...
export const removeAnnouncement = async (req, res) => {
    try {
        ...
        const announcement = await deleteAnnouncement(req.params['id'], req.user.campus);
        if (!announcement) {
            res.status(HttpStatus.NOT_FOUND).json({ message: 'Announcement not found or not in your campus.' });
            return;
        }
